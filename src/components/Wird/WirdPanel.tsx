...
import { Button, Drawer, TabPanel, Tabs } from '../ui'
...
function GradeRow({ onGrade }: { onGrade: (g: Grade) => void }) {
  return (
    <div className="wird-grades">
      {GRADES.map((g) => (
        <Button key={g} className={`wird-grade wird-grade--${g}`} size="sm" onClick={() => onGrade(g)}>
          {GRADE_LABEL[g]}
        </Button>
      ))}
    </div>
...
export function WirdPanel({ onClose, onGoPage }: Props) {
  ...
  const tabs = TABS.map((item) => ({
    ...item,
    badge: item.id === 'review' && due.length > 0 ? arNum(due.length) : undefined,
  }))
  ...
  const onAddManual = (e: FormEvent) => {
    e.preventDefault()
    setFormError(
      s.addHifz(mSurah, Number.parseInt(mFrom, 10), Number.parseInt(mTo, 10), surahTotal(mSurah)),
    )
  }
  ...
  return (
    <Drawer
      ...
      description="قراءة وحفظ ومراجعة في مساحة واحدة هادئة."
      onClose={onClose}
      size="lg"
      className="wird-v2"
    >
      <Tabs items={tabs} value={tab} onChange={setTab} label="أقسام رحلتي" idBase={TABS_ID} />
      <TabPanel idBase={TABS_ID} tabId={tab} className="wird-body">
        {tab === 'today' && (
          <div className="wird-today">
                ...
                pt.done ? (
                  ...
                  <>
                    ...
                    <p className="wird-sub">
                      أنت عند صفحة {arNum(plan.position)} — بقي{' '}
                      {arNum(Math.max(0, pt.targetEnd - plan.position + 1))} صفحات
                    </p>
                    <button
            ...
            <section className="wird-card wird-today-card">
              ...
              {slice ? (
                backlog ? (
                  <p className="wird-error">
                    أنجز مستحقات المراجعة ({arNum(due.length)}) أولاً، ثم أكمل الحفظ
                  </p>
                ) : (
                  <>
            ...
            <section className="wird-card wird-today-card">
              ...
              {due.length === 0 ? (
                <p className="wird-sub">
                  لا مستحقات —{' '}
                  {upcoming.length > 0
                    ? `أقرب مراجعة ${upcoming[0].dueDate}`
                    : 'تُنشأ تلقائياً من تقييمات الحفظ'}
                </p>
              ) : (
                due.map((h) => (
          ...
          <div className="wird-plans">
            <section>
              ...
              {plan && pt ? (
                <>
                  ...
                  <div className="wird-trackbar">
                    <div
                      className="wird-trackbar-fill"
                      style={{ width: `${((plan.position - 1) / 604) * 100}%` }}
                    />
                  </div>
                  <div className="wird-range">
                    ...
                    <button
                      ...
                      disabled={!resetVal}
                      onClick={() => {
                        s.resetPosition(Number(resetVal))
                        setResetVal('')
                      }}
                    >
                      ضبط
                    </button>
                    <button className="reader-btn" onClick={s.stopPlan}>
                      إيقاف الخطة
                    </button>
                  </div>
                </>
              ...
              <p className="wird-hint">
                ٣٠ يوماً ≈ جزءاً يومياً · الخطة مرنة: يوم فائت يُعاد توزيعه تلقائياً
              </p>
            </section>
              ...
              {s.hifzTrack ? (
                <>
                  <p className="wird-sub">
                    ...
                    — السورة {arNum(s.hifzTrack.index + 1)} من {arNum(s.hifzTrack.queue.length)} ·{' '}
                    {arNum(s.hifzTrack.pace)} آيات للمقطع
                  </p>
                  <div className="wird-goal">
                    ...
                    {PACES.map((p) => (
                      <button
                        key={p}
                        className={'reader-btn' + (s.hifzTrack!.pace === p ? ' wird-goal--active' : '')}
                        onClick={() => s.setPace(p)}
                      >
                        {arNum(p)}
                      </button>
                    ))}
                    <button className="reader-btn" onClick={s.stopTrack}>
                      إيقاف المسار
                    </button>
                  </div>
                </>
                ...
                <>
                  <div className="wird-modes">
                    ...
                    <div className="wird-mode wird-mode--custom">
                      ...
                      <div className="wird-range">
                        <select
                          className="reader-btn wird-select"
                          value={customSurah}
                        ...
                        <button
                          className="reader-btn"
                          onClick={() => s.startTrack('custom', pace, customSurah)}
                        >
                          ابدأ
                        </button>
                      </div>
                  ...
                  <div className="wird-goal">
                    ...
                    {PACES.map((p) => (
                      <button
                        key={p}
                        className={'reader-btn' + (pace === p ? ' wird-goal--active' : '')}
                        onClick={() => setPace(p)}
                      >
                        {arNum(p)} آيات
                      </button>
            ...
            <section>
              ...
              <form className="wird-form" onSubmit={onAddManual}>
                <select
                  className="reader-btn wird-select"
                  value={mSurah}
                  onChange={(e) => setMSurah(Number(e.target.value))}
                >
                  {names.map((n) => (
                    <option key={n.n} value={n.n}>
                      {arNum(n.n)}. {n.name}
                ...
                <div className="wird-range">
                  ...
                  <button type="submit" className="reader-btn">
                    إضافة
                  </button>
                </div>
        ...
        {tab === 'stats' && (
          <div className="wird-stats">
              ...
              {(() => {
                ...
                return (
                  <>
                    ...
                    <p className="wird-sub">
                      أطول سلسلة: {arNum(st.longest)} · أي إنجاز (قراءة/حفظ/مراجعة) يحفظها
                    </p>
                  </>
                )
              })()}
            ...
            <section className="wird-card wird-today-card">
              ...
              <p className="wird-hint">
                <i className="tree-key tree-cell--1" /> بدأ · <i className="tree-key tree-cell--2" /> أتم
                الحفظ · <i className="tree-key tree-cell--3" /> أتقن — انقر سورة لفتحها
              </p>
            </section>
            ...
            <section className="wird-card wird-today-card">
              ...
              <div className="wird-range wird-backup-row">
                ...
                <label className="reader-btn icon-btn" title="استيراد نسخة من ملف">
                  <IconUpload />
                  <input
                    type="file"
              ...
              {isTauri() && (
                <p className="wird-hint">
                  السحابية تُحفظ في مجلد المزامنة وتُرفع تلقائياً — على الجهاز الآخر: نزّلها ثم «استيراد
                  نسخة»
                </p>
              )}
        ...
        {tab === 'review' && (
          <div className="wird-review">
            {due.length === 0 && upcoming.length === 0 && s.hifz.length === 0 ? (
              <p className="wird-hint wird-empty">
                لا عناصر بعد — ابدأ مسار الحفظ وستجد مراجعاتك هنا تلقائياً
              </p>
            ) : (
              <>
                {due.length > 0 && (
                  <section>
                    ...
                    {due.map((h) => (
                      <div key={h.id} className="wird-card wird-card--due">
                        <span className="wird-item-title">
                          {sliceText(names, h.surah, h.from, h.to)}
                        </span>
                        <span className="wird-sub">مستحقة منذ {h.dueDate}</span>
                        <GradeRow onGrade={(g) => s.grade(h.id, g)} />
                      </div>
                  ...
                  <section>
                    ...
                    {upcoming.map((h: HifzItem) => (
                      <div key={h.id} className="wird-card wird-card--muted">
                        ...
                        <span className="wird-sub">
                          {h.dueDate} · آخر تقييم {h.lastGrade ? GRADE_LABEL[h.lastGrade] : '—'}
                        </span>
                        <button

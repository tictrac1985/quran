...
export const loadMeta = () => (metaCache ??= fetchJson<MushafMeta>(META_PATH))
...
export const juzLabel = (n: number): string =>
  n >= 1 && n <= 30 ? `الجزء ${JUZ_ORDINALS[n - 1]}` : ''
...
export const hizbOfPage = (meta: MushafMeta, page: number): number =>
  entryOfPage(meta.hizb, page)
export const rubOfPage = (meta: MushafMeta, page: number): number =>
  entryOfPage(meta.rub, page)
